...
class Session():
    def __init__(self, config, net, db):
        self.config = config
        self.net = net 
        self.db = db
        self.crypto = Crypt()
    ...
    def login(self, username):
        ...
        if not user:
            self.net.send_response(False)
            return False
        else:
            self.net.send_response(True)
        self.crypto.load_user_public_key(user)
        self.net.send_data(self.crypto.generate_login_cipher())
